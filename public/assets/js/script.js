...
// $(document).ready(function(){
	...
	function get_brands(){
		...
		$.ajax({
			...
			type:"GET",
			url:"get_subcategory/"+cate_id,
			// data:{id : cate_id},
			dataType:"Json",
			success:function(response){
				if(Object.keys(response['brands']).length!=0){
					// console.log(response['brands'][0]['sub_cate_name'])
					for(i = 0 ;i<Object.keys(response['brands']).length;i++){
						var option = document.createElement("option");
						option.text=response['brands'][i]['sub_cate_name']
						option.value = response['brands'][i]['sub_cate_id']						
						sub_cate_filed.append(option,sub_cate_filed[i+1])
					}						
				}
				else{
					console.log("shfs");
					sub_cate_filed.innerHTML="<option> No sub cateogiry found</option>"
				...
				$.ajax(
					{
						...
						success: function (response){
							swal({                                 
								title: 'Deleted!',                                 
								text: 'producted has been deleted.',                                 
								type: 'success',                                 
								timer: 2000,                             
							});       
